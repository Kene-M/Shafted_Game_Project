...
func init():
	...
	var texture = load("res://Assets/Player/shotgun.png")
	var char = get_parent()
	...
	if $"../AnimatedSprite2D".flip_h == false:
		sprite.position = Vector2(-3, 7)
	else:
		sprite.position = Vector2(3, 7)
	sprite.rotation = 0
	
func _add_projectile(direction, augment_vals):
	print("Proj Fire")
	var projectile = preload("res://Scenes/weaponScenes/shotgun_projectile.tscn")
	...
	proj_inst.base_damage = (25 + augment_vals[AugType.Type.ATKADD]) * augment_vals[AugType.Type.ATKMULT]
...
func _on_fire_projectile(direction, augment_vals) -> void:
	if (Time.get_ticks_msec() - last_used > 400) or last_used == 0:
		last_used = Time.get_ticks_msec()
		_add_projectile(direction, augment_vals)
		_add_projectile(direction.rotated(deg_to_rad(15)), augment_vals)
		_add_projectile(direction.rotated(deg_to_rad(7)), augment_vals)
		_add_projectile(direction.rotated(deg_to_rad(-15)), augment_vals)
		_add_projectile(direction.rotated(deg_to_rad(-7)), augment_vals)
